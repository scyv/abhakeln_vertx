class AbhakelnEventBus {
  ...
  reset() {
    const eventbus = new EventBus(this.endpoint, {
      vertxbus_reconnect_attempts_max: 20,
    ...
    switch (action) {
      ...
      case "update-item-data": {
        ...
        if (localItem) {
          ...
          if (localItem.reminder !== decrypted.reminder && decrypted.reminder && decrypted.reminder !== "") {
            new Notifications().triggerTaskReminder(decrypted);
          }
